# MGraph AI Service Proxy - Setup Guide

This guide walks through the complete setup process from initial repository creation to v1.0.0 release.

## 📋 Prerequisites

- GitHub repository created: `https://github.com/the-cyber-boardroom/MGraph-AI__Service__Proxy`
...
git init
echo "# MGraph AI Service Proxy" > temp.md
git add temp.md
git commit -m "Initial repository creation"
git branch -M main
git remote add origin https://github.com/the-cyber-boardroom/MGraph-AI__Service__Proxy.git
...
git add .
git commit -m "First commit for MGraph AI Service Proxy - v1.0.0 scaffold with FastAPI setup, tests, and CI/CD pipeline"
...
3. **Verify AWS Lambda**:
   ...
   - Look for `mgraph_ai_service_proxy-dev`
...
3. **Test the domain**:
   ...
   # Should return: {"status":"healthy","service":"mgraph_ai_service_proxy"}
   ```
...
3. **Verify QA deployment**:
   ...
   - Lambda function `mgraph_ai_service_proxy-qa` should be created
...
2. **Verify production deployment**:
   - Lambda function `mgraph_ai_service_proxy-prod` should be created
...
echo "v1.0.0" > mgraph_ai_service_proxy/version
...
# Commit changes
git add mgraph_ai_service_proxy/version README.md pyproject.toml
...
echo "   1. Go to https://github.com/the-cyber-boardroom/MGraph-AI__Service__Proxy/releases"
...
This completes the setup! Your MGraph AI Service Proxy is now fully operational across all environments.