...
print_step "Renaming service directory from mgraph_ai_service_proxy to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_proxy" ]; then
    mv mgraph_ai_service_proxy "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_proxy directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service Proxy" with the new service title
replace_in_files "MGraph AI Service Proxy" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_proxy" with the new package name
replace_in_files "mgraph_ai_service_proxy" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__Proxy" with the new repository name
replace_in_files "MGraph-AI__Service__Proxy" "$REPO_NAME" "repository name"
